...
"""Materialize the preregistered V25 heterogeneous parameter sets."""
...
FAMILY_PATH = PROJECT_DIR / "configs/v25_heterogeneous_family.yaml"
OUTPUT_ROOT = PROJECT_DIR / "artifacts/V25_heterogeneous_validation/parameter_sets"
...
def materialize(unit: dict, family: dict, calibration_base: dict, dispatch_base: dict) -> dict:
    ...
    calibration["metadata"]["version"] = "V25-heterogeneous-family"
    ...
    for state, mean_kw in family["load_levels"][unit["load"]].items():
        calibration["load_calibration"]["state_targets"][state][
            "target_mean_kw"
        ] = float(mean_kw)
    plant = family["plant_levels"][unit["plant"]]
    calibration["v25_risk_overrides"] = deepcopy(plant["risk_overrides"])
    calibration["v25_unit"] = deepcopy(unit)

    dispatch = deep_merge(dispatch_base, plant["dispatch_overrides"])
    dispatch["metadata"]["version"] = "V25-heterogeneous-family"
