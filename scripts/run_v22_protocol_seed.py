...
"""Run one prospective V22 seed in the preregistered queue.
...
DISPATCH_CONFIG = PROJECT_DIR / "configs/v22_dispatch_matched.yaml"
...
def _verify_queue(seed: int, protocol: dict, artifact_root: Path) -> None:
    ...
    if seed not in queue:
        raise ValueError(f"seed {seed} is not in the frozen V22 holdout queue")
    retained = 0
    ...
    for earlier in queue[: queue.index(seed)]:
        ...
        if not eligibility_path.exists():
            raise RuntimeError(f"process earlier V22 seed {earlier} first")
        eligibility = json.loads(eligibility_path.read_text(encoding="utf-8"))
        if eligibility.get("eligible"):
            retained += 1
    if retained >= required:
        raise RuntimeError(
            f"the first {required} eligible V22 seeds are already complete"
        )
...
def _verify_freeze(seed: int, protocol_path: Path, artifact_root: Path) -> dict:
    ...
    if not freeze_path.exists():
        raise RuntimeError("V22 is not frozen; refusing prospective execution")
    manifest = json.loads(freeze_path.read_text(encoding="utf-8"))
    if manifest.get("protocol_sha256") != _sha256(protocol_path):
        raise RuntimeError("V22 protocol changed after freeze")
    if seed not in [int(value) for value in manifest["prospective_seed_queue"]]:
        raise RuntimeError("seed is absent from the frozen V22 queue")
    changed = []
    ...
    if changed:
        raise RuntimeError("V22 frozen files changed: " + ", ".join(changed))
    return manifest
...
def _prepare_inputs(
    ...
    data_path = PROJECT_DIR / f"data/processed/rig_load_v22_{seed}.parquet"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one frozen V22 holdout seed")
    ...
    if transition.get("source") != "history_transition_flags" or not bool(
        ...
        raise RuntimeError("V22 risk features failed the causal-information gate")
