...
PROTOCOL_PATH = PROJECT_DIR / "configs/v12_causal_deficit_commitment.yaml"
DIAGNOSTIC_SOURCES = {
...
def _run_seed(seed: int, source: Path, supervisor: dict) -> dict:
    output = (
        PROJECT_DIR
        / "artifacts/v12_causal_deficit_commitment"
        / f"diagnostic_seed_{seed}"
    )
    ...
    expected_trajectories = list(dispatch_dir.glob("trajectory_*.csv"))
    if metrics_path.exists() and len(expected_trajectories) == 21:
        print("REUSE COMPLETE DIAGNOSTIC", dispatch_dir, flush=True)
    else:
    ...
    for scenario in proposed.index:
        ...
        trigger_counts[scenario] = {
            "guard_evaluated_steps": int(
                trajectory["low_soc_commitment_guard_evaluated"].sum()
            ),
            "guard_triggered_steps": int(
                trajectory["low_soc_commitment_guard_triggered"].sum()
            ),
        }
...
def main() -> None:
    ...
    summary = {
        ...
        "single_change_from_v11": "causal_forecast_deficit_guard_for_two_unit_floor",
        "results": results,
    ...
    output = (
        PROJECT_DIR
        / "artifacts/v12_causal_deficit_commitment/diagnostic_summary.json"
    )
