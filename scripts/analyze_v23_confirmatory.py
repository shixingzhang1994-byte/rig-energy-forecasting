...
"""Aggregate the first 12 eligible V23 seeds at the seed-cluster level."""
...
def _retained_seed_roots(protocol: dict, artifact_root: Path) -> list[Path]:
    ...
    for seed in map(int, protocol["prospective_holdout_seed_order"]):
        ...
        if not eligibility_path.exists():
            raise RuntimeError(f"V23 queue is incomplete at seed {seed}")
        eligibility = json.loads(eligibility_path.read_text(encoding="utf-8"))
        if not eligibility.get("eligible"):
            continue
        if not (root / "result.json").exists():
            raise RuntimeError(f"eligible V23 seed {seed} is incomplete")
        retained.append(root)
        if len(retained) == required:
            return retained
    raise RuntimeError(f"only {len(retained)} eligible V23 seeds are complete")
...
def main() -> None:
    ...
    for root in roots:
        ...
        if not bool(result.get("technical_gate_pass")):
            failed.append(
                {
                    "seed": int(root.name.split("_")[-1]),
                    "failure_stage": result.get("failure_stage", "technical_gate"),
                    "failure_type": result.get("failure_type", "gate_failure"),
                }
            )
    if failed:
        manifest = {
            "protocol_id": protocol["id"],
            "status": "technical_gate_failure_no_efficacy_estimates",
