...
"""Run one frozen V27 heterogeneous parameter-set-by-seed unit."""
...
PROTOCOL_PATH = PROJECT_DIR / "configs/v27_heterogeneous_validation.yaml"
...
def parameter_record(seed: int) -> tuple[dict, Path]:
    ...
    if len(matches) != 1:
        raise ValueError(f"seed {seed} has no unique V27 parameter assignment")
    record = matches[0]
    ...
    if implementation._sha256(calibration_path) != record["calibration_sha256"]:
        raise RuntimeError("V27 materialized calibration hash mismatch")
    if implementation._sha256(dispatch_path) != record["dispatch_sha256"]:
        raise RuntimeError("V27 materialized dispatch hash mismatch")
...
def prepare_inputs(
    ...
    forecast["metadata"] = {"v27_parameter_unit": record["unit_id"]}
    ...
    risk = implementation.legacy_runner._deep_merge(
        risk, calibration.get("v27_risk_overrides", {})
    )
    ...
    risk["metadata"] = {"v27_parameter_unit": record["unit_id"]}
    risk_config = root / "source/resolved_risk_config.yaml"
    implementation.legacy_runner._write_yaml(risk, risk_config)

    data_path = PROJECT_DIR / f"data/processed/rig_load_v27_{seed}.parquet"
...
def _augment_result_with_deadline_audit(seed: int, root: Path) -> None:
    ...
    if not result_path.exists() or not audit_path.exists():
        raise RuntimeError("V27 result or deadline audit is missing")
