...
"""Freeze the V25 heterogeneous family before any prospective unit outcome."""
...
PROTOCOL = PROJECT_DIR / "configs/v25_heterogeneous_validation.yaml"
FAMILY = PROJECT_DIR / "configs/v25_heterogeneous_family.yaml"
PARAMETER_ROOT = PROJECT_DIR / "artifacts/V25_heterogeneous_validation/parameter_sets"
DEVELOPMENT = PROJECT_DIR / "artifacts/V25_heterogeneous_validation/development/seed_20261100"
...
def main() -> None:
    ...
    if len(prospective) != 12 or len(factors) != 12:
        raise RuntimeError("V25 factor design is not the declared balanced 3x2x2 family")
    ...
    if revealed:
        raise RuntimeError(f"prospective V25 unit directories already exist: {revealed}")
    ...
    implementation.RUNNER_PATHS = [
        PROJECT_DIR / "scripts/prepare_v25_parameter_sets.py",
        PROJECT_DIR / "scripts/run_v25_protocol_seed.py",
        PROJECT_DIR / "scripts/run_v25_development.py",
        FAMILY,
        *parameter_files,
    ]
    implementation.ANALYSIS_PATHS = [
        PROJECT_DIR / "scripts/analyze_v25_heterogeneous.py",
    ]
