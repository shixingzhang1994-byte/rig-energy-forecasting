...
"""Freeze the V22 implementation after development and before holdout replay."""
...
from run_v22_protocol_seed import audit_startup_delay  # noqa: E402


PROTOCOL_PATH = PROJECT_DIR / "configs/v22_submission_revision.yaml"
DEVELOPMENT_ROOT = (
    PROJECT_DIR / "artifacts/V22_submission_revision/development/seed_20261008"
)
...
def main() -> None:
    ...
    if not metrics_path.exists() or not calibration_path.exists():
        raise RuntimeError("V22 development dispatch/calibration is incomplete")

    metrics = pd.read_csv(metrics_path)
    expected_methods = set(document["matched_comparison"]["methods"])
    if set(metrics["method"]) != expected_methods:
        raise RuntimeError("V22 development method set differs from protocol")
    if metrics.groupby("scenario")["method"].nunique().min() != len(expected_methods):
        raise RuntimeError("V22 development method-by-scenario matrix is incomplete")
    ...
    if not development_result["technical_gate_pass"]:
        raise RuntimeError("V22 development technical gate failed; refusing freeze")

    frozen_paths = [
        PROTOCOL_PATH,
        PROJECT_DIR / "configs/v22_dispatch_matched.yaml",
        ...
        PROJECT_DIR / "scripts/run_v22_protocol_seed.py",
        PROJECT_DIR / "scripts/analyze_v22_confirmatory.py",
    ...
    if freeze_path.exists():
        raise RuntimeError("V22 freeze manifest already exists; refusing overwrite")
    manifest = {
        "protocol_id": protocol["id"],
        "status": "frozen_before_any_prospective_V22_outcome",
